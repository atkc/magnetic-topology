...
xx=zt;%[zt zb];                                    % Combine the body and top coordinates
yy=yt;%[yt yb];                                    % Combine the body and top coordinates
zz=xb;%[xt xb];                                    % Combine the body and top coordinates

vertices=[xx(:),yy(:),zz(:)];
faces=convhulln(vertices);

% Generate the REAL surface data for rendering
[xt,yt,zt]=cylinder(linspace(0.7*S,0,2),max(N));% Arrow tip, edit scale to change slope
...
xa=zt;%[zt zb];  % Final arrow coordinates (tip+body)
ya=yt;%[yt yb];
za=xt;%[xt xb];
