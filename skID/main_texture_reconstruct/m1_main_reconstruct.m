...
filename='200207_x26t2_mfm0001_rot180custom';
ext='.png';
filepath='C:\Users\ant_t\Desktop\mumax3.9c\mfm_mag_26t2\200207_26t2_recon\';
rawIm=imread([filepath filename ext]);
portion=16;%always input 2^x

[sizex,sizey]=size(rawIm);
dx=sizex/sqrt(portion);
dy=sizey/sqrt(portion);
im_mat=zeros(dx,dy,portion);
im_matz=1;
for portionix=1:sqrt(portion)
    for portioniy=1:sqrt(portion)
    	im_cut=double(rawIm((dx*(portionix-1)+1):dx*portionix,(dy*(portioniy-1)+1):dy*portioniy))/double(max(rawIm(:)));
%         figure;
%         imshow(im_cut)
        im_mat(:,:,im_matz)=im_cut;
        im_matz=im_matz+1;
    end
...
for portioni=1:portion
    nowIm=(reshape(im_mat(:,:,portioni),[dx,dy]));
    if InvertInd
        nowIm = imcomplement(nowIm);
    end
    figure
    imshow(nowIm)
    im_length=grid_xy_size;
    nowIm = imresize(nowIm,im_length/length(nowIm),'bicubic');
    figure
    imshow(nowIm);
    ...
    imageSize=20; %for gauging size filter (um)
    ...
    [dgrayIm, filIm, binIm1, binIm2, binIm3, centroids,threshVal]=m1_binarize(double(nowIm),threshOpt,threshVal,adaptArea,erodeSize,filRpt,filSize,minSize,maxSize,c_th,e_th,imageSize,connect,chop);
    se = strel('square',1);
...
%     plot_rgb_vec(imxBi,imyBi,imzBi);
    imwrite(binIm1,strcat(filename,'_',num2str(portioni),'_bi.png'));
    imwrite(nowIm,strcat(filename,'_',num2str(portioni),'_raw.png'));
    ovf_filename=strcat(filename,'_',num2str(portioni),'.ovf');
