function [isofit,unfitno,fit_status,er]=m2_fit2d(radius,centroids, dgrayIm, name,maxr,checkInd,imageSize,maskId)
%Description:
%This version of m2_fit2d fits a WEIGHTED GAUSSIAN (by it's normalized self) to dgrayIm
%outputs:isofit(i,1:4) = [xo+double(x1)-1 yo+double(y1)-1 sigma sigma*2*(2*log(2))^0.5];
...
%     xyfit = zeros(length(center),7); %store aniso fit
    isofit =zeros(mx,5); %store iso fit
    fit_status=zeros(mx,1);
...
%         a=tmpiso.a;
    ...
    fun=@(xt)(Gfun2D(size(portion),xt(1),xt(2),xt(3),xt(4),xt(5))-portion).*(Gfun2D(size(portion),xt(1),xt(2),xt(3),0,xt(5))/(xt(5)));
    if maskId
...
%     ratio=x(3)/lb(3)
    er=resnorm/numel(residual);%resnorm is the squared 2-norm of the residual at x: sum(fun(x).^2).
    xo=x(1);
    yo=x(2);
    sigma=x(3);
    b=x(4);
    a=x(5);
    isofit(i,1:4) = [xo+double(x1)-1 yo+double(y1)-1 sigma sigma*2*(2*log(2))^0.5];
