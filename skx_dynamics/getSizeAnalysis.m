...
% %**********Analysis 3a: HA vs Size (for all)**************
% %**********Analysis 3b: V vsc Size (for all)**************
% [~,jlist_id]=ismember(p_id,p_sub);
% i_list=(abs(i1(jlist_id)));
% i_list=unique(i_list);
% for n2=5:15
...
% f4=figure;
% i_use=1;
% size_edge=linspace(70,200,n2);
% fil_para.sel_p=p_id;
% [vsk_flow,theta_flow,sksize_flow,p_no_hold] = data_filter(r_cor,theta_cor,sksize,p_no,fil_para);

% figure(f3);
% [jA3,j_edgeA3,sksizeA3,sksize_edgeA3,thetaA3,countsA3]=bin3D(i_use*ones(size(sksize_flow)),sksize_flow,theta_flow,1,n2);
% thetaA3(countsA3<c1)=nan;
% plot(sksizeA3,thetaA3,'-o','DisplayName','all j_f_l_o_w');
% hold on
% figure(f4);    
% [jA3,j_edgeA3,sksizeA3,sksize_edgeA23,vA3,countsA3]=bin3D(i_use*ones(size(sksize_flow)),sksize_flow,vsk_flow,1,size_edge);
% vA3(countsA3<c1)=nan;
% plot(sksizeA3,vA3,'-o','DisplayName','all j_f_l_o_w');
% xlabel('Size(nm)')
% ylabel('V_s_k(m/s)')
% hold on
% end
